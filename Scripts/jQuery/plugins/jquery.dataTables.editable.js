...
(function ($) {
    ...
    $.fn.makeEditable = function (options) {
        ...
        function _fnStartProcessingMode() {
            ...
            if (oTablaEditable.fnSettings().oFeatures.bProcessing) {
        ...
        function fnApplyEditable(aoNodes) {
            ...
            var oDefaultEditableSettings = {
                ...
                "submitdata": function (value, settings) {
                    ...
                    var rowId = oTablaEditable.fnGetPosition(this)[0];
                    var columnPosition = oTablaEditable.fnGetPosition(this)[1];
                    var columnId = oTablaEditable.fnGetPosition(this)[2];
                    var sColumnName = oTablaEditable.fnSettings().aoColumns[columnId].sName;
                    if (sColumnName == null || sColumnName == "")
                        sColumnName = oTablaEditable.fnSettings().aoColumns[columnId].sTitle;
                    var updateData = null;
                ...
                "callback": function (sValue, settings) {
                    ...
                    var status = "";
                    var aPos = oTablaEditable.fnGetPosition(this);
                    
                    var bRefreshTable = !oSettings.oFeatures.bServerSide;
                    $("td.last-updated-cell", oTablaEditable.fnGetNodes( )).removeClass("last-updated-cell");
                    if(sValue.indexOf(properties.sFailureResponsePrefix)>-1)
                    {
                        oTablaEditable.fnUpdate(sOldValue, aPos[0], aPos[2], bRefreshTable);
                        $("td.last-updated-cell", oTablaEditable).removeClass("last-updated-cell");
                        ...
                        if (properties.sSuccessResponse == "IGNORE" || 
                            ...
                            if(sNewCellDisplayValue == null)
                            {
                                ...
                                oTablaEditable.fnUpdate(sValue, aPos[0], aPos[2], bRefreshTable);
                            }else{
                                oTablaEditable.fnUpdate(sNewCellDisplayValue, aPos[0], aPos[2], bRefreshTable);
                            }
                            $("td.last-updated-cell", oTablaEditable).removeClass("last-updated-cell");
                            $(this).addClass("last-updated-cell");
                            status = "success";
        ...
        function fnOnRowAdded(data) {
            ...
            if (properties.fnOnNewRowPosted(data)) {
                ...
                var oSettings = oTablaEditable.fnSettings();
                if (!oSettings.oFeatures.bServerSide) {
                    ...
                    if (oSettings.aoColumns != null && isNaN(parseInt(oSettings.aoColumns[0].mDataProp))) {
                        rtn = oTablaEditable.fnAddData(rowData);
                    }
                    else {
                        rtn = oTablaEditable.fnAddData(values);
                    }

                    var oTRAdded = oTablaEditable.fnGetNodes(rtn);
                    //add id returned by server page as an TR id attribute
                    properties.fnSetRowID($(oTRAdded), data, true);
                    //Apply editable plugin on the cells of the table
                    fnApplyEditable(oTRAdded);

                    $("tr.last-added-row", oTablaEditable).removeClass("last-added-row");
                    $(oTRAdded).addClass("last-added-row");
                } /*else {
                    oTablaEditable.fnDraw(false);
                }*/
                //Close the dialog
        ...
        function _fnOnRowDelete(event) {
            ...
            if (!properties.bUseKeyTable) {
                if ($('tr.' + properties.sSelectedRowClass + ' td', oTablaEditable).length == 0) {
                    //oDeleteRowButton.attr("disabled", "true");
                    _fnDisableDeleteButton();
                    return;
                }
                nSelectedCell = $('tr.' + properties.sSelectedRowClass + ' td', oTablaEditable)[0];
            } else {
                nSelectedCell = $('td.focus', oTablaEditable)[0];
            ...
            if (properties.bUseKeyTable) {
                oKeyTablePosition = oTablaEditable.keys.fnGetCurrentPosition();
            }
            var id = fnGetCellID(nSelectedCell);
...
/*
            if (!properties.bUseKeyTable) {
                oTRSelected = $('tr.' + properties.sSelectedRowClass, oTablaEditable)[0];
            } else {
                oTRSelected = $("td.focus", oTablaEditable)[0].parents("tr")[0];
            }
            */
            if (response == properties.sSuccessResponse || response == "") {
                oTablaEditable.fnDeleteRow(oTRSelected);
                fnDisableDeleteButton();
                fnSetDisplayStart();
                if (properties.bUseKeyTable) {
                    oTablaEditable.keys.fnSetPosition( oKeyTablePosition[0], oKeyTablePosition[1] ); 
                }
        ...
        */
        ...
        function fnGetActionSettings(sAction) {
            ...
            if (properties.aoTablaEditableAction)
                properties.fnShowError("Configuration error - aoTablaEditableAction setting are not set", sAction);
            var i = 0;

            for (i = 0; i < properties.aoTablaEditableActions.length; i++) {
                if (properties.aoTablaEditableActions[i].sAction == sAction)
                    return properties.aoTablaEditableActions[i];
            }
        ...
        function fnPopulateFormWithRowCells(oForm, oTR) {
            ...
            var iRowID = oTablaEditable.fnGetPosition(oTR);
            ...
            $("input:text[rel],input:radio[rel][checked],input:hidden[rel],select[rel],textarea[rel],input:checkbox[rel]",
                                    $(oForm)).each(function () {
                                        ...
                                        if (rel >= iColumnCount)
                                            ...
                                            var sCellValue = oTablaEditable.fnGetData(oTR)[rel];
                                            ...
                                            //oTablaEditable.fnUpdate(sCellValue, iRowID, rel);
                                        }
                                    });
    ...
    function fnUpdateRowOnSuccess(nActionForm) {
        ...
        oSettings = oTablaEditable.fnSettings();
        properties.bUseKeyTable = (properties.oKeyTable != null);

        return this.each(function () {
            var sTableId = oTablaEditable.dataTableSettings[0].sTableId;
            //KEYTABLE
            if (properties.bUseKeyTable) {
                var keys = new KeyTable({
                    "table": document.getElementById(sTableId),
                    "datatable": oTablaEditable
                });
                oTablaEditable.keys = keys;
            ...
            if (oTablaEditable.fnSettings().sAjaxSource != null) {
                oTablaEditable.fnSettings().aoDrawCallback.push({
                    "fn": function () {
                        //Apply jEditable plugin on the table cells
                        fnApplyEditable(oTablaEditable.fnGetNodes());
                        $(oTablaEditable.fnGetNodes()).each(function () {
                            var position = oTablaEditable.fnGetPosition(this);
                            var id = oTablaEditable.fnGetData(position)[0];
            ...
            $(".table-action-deletelink", oTablaEditable).on("click", _fnOnRowDeleteInline);

            if (!properties.bUseKeyTable) {
            //Set selected class on row that is clicked
            //Enable delete button if row is selected, disable delete button if selected class is removed
            $("tbody", oTablaEditable).click(function (event) {
                if ($(event.target.parentNode).hasClass(properties.sSelectedRowClass)) {
                    ...
                    $(oTablaEditable.fnSettings().aoData).each(function () {
                ...
                oTablaEditable.keys.event.focus(null, null, function (nNode, x, y) {

                });
            }

            if (properties.aoTablaEditableActions != null) {
                for (var i = 0; i < properties.aoTablaEditableActions.length; i++) {
                    var oTablaEditableAction = $.extend({ sType: "edit" }, properties.aoTablaEditableActions[i]);
                    var sAction = oTablaEditableAction.sAction;
                    var sActionFormId = oTablaEditableAction.sActionFormId;

                    var oActionForm = $("#form" + sAction);
                    if (oActionForm.length != 0) {
                        var oFormOptions = { autoOpen: false, modal: true };
                        oFormOptions = $.extend({}, oTablaEditableAction.oFormOptions, oFormOptions);
                        oActionForm.dialog(oFormOptions);
                        oActionForm.data("action-options", oTablaEditableAction);
                        ...
                        if (oActionFormLink.length != 0) {
                            ...
                            oActionFormLink.on("click", function () {
                                ...
                                var oTablaEditableAction = $(sActionFormId).data("action-options");

                                if (oTablaEditableAction.sType == "edit") {
                ...
                } // end for (var i = 0; i < properties.aoTablaEditableActions.length; i++)
            } //end if (properties.aoTablaEditableActions != null)
